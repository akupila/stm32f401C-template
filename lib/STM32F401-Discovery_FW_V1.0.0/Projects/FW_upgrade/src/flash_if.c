...
uint32_t FLASH_If_EraseSectors(uint32_t Address)
{
  ...
  while ((erasestatus == FLASH_COMPLETE) && (sectorindex <= FLASH_Sector_5) && (HCD_IsDeviceConnected(&USB_OTG_Core) == 1))
  {
    erasestatus = FLASH_EraseSector(sectorindex, VoltageRange_3);
    sectorindex += 8;
  }
  
  if ((sectorindex != (FLASH_Sector_5 + 8)) || (erasestatus != FLASH_COMPLETE))
  {
    return 1;
  }
  else
